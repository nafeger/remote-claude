...
- [x] 6.0 유닛 테스트 구현
  ...
  - [x] 6.5 `src/handlers/__tests__/mention-filter.test.ts` 생성 및 `filterSlackMentions()` 테스트
...
- [x] 7.0 시스템 테스트 구현
  - [x] 7.1 `src/__tests__/system/dsl-workflow.test.ts` 파일 생성
  - [x] 7.2 시스템 테스트 1: 간단한 선택
    - [x] 7.2.1 선택 UI를 표시하는 테스트 프롬프트로 Claude Code 시작
    - [x] 7.2.2 사용자가 Slack을 통해 `e` 전송 시뮬레이션
    - [x] 7.2.3 검증: 명령 파싱, Enter 전송, 화면 캡처, Slack 메시지 업데이트
    - [x] 7.2.4 응답 시간 측정 및 ≤3초 검증
  - [x] 7.3 시스템 테스트 2: 혼합 입력
    - [x] 7.3.1 텍스트 입력 필요 프롬프트로 Claude Code 트리거
    - [x] 7.3.2 사용자가 `ddd` custom value `e` 전송 시뮬레이션
    - [x] 7.3.3 검증: Down×3, "custom value" 텍스트, Enter 전송, 화면 캡처
    - [x] 7.3.4 응답 시간 측정 및 ≤3초 검증
  - [x] 7.4 시스템 테스트 3: 혼합 문자 에러
    - [x] 7.4.1 사용자가 `ddx` 전송 시뮬레이션
    - [x] 7.4.2 검증: 에러 메시지 표시, 명령 실행 안 됨, 가이드 제공
    - [x] 7.4.3 사용자가 `dd` 재전송 시뮬레이션
    - [x] 7.4.4 검증: Down×2 정상 실행
  - [x] 7.5 시스템 테스트 4: Slack 멘션 필터링 (FR9)
    - [x] 7.5.1 사용자가 "@홍길동 @file.ts 파일 수정" 입력 시뮬레이션
    - [x] 7.5.2 Slack이 "<@U12345> @file.ts 파일 수정"으로 변환
    - [x] 7.5.3 검증: Slack 멘션 감지, 필터링, 사용자 알림
    - [x] 7.5.4 검증: "@file.ts 파일 수정"만 Claude Code로 전송
    - [x] 7.5.5 검증: @file.ts 파일 참조 정상 작동
  - [x] 7.6 시스템 테스트 5: 기본 입력 처리 (FR10)
    - [x] 7.6.1 사용자가 "/ask" 없이 "implement feature X" 입력 시뮬레이션
    - [x] 7.6.2 검증: 자동으로 Claude Code에 요청 전송
    - [x] 7.6.3 사용자가 "이 코드 설명해줘" 입력 시뮬레이션
    - [x] 7.6.4 검증: 자동으로 Claude Code에 요청 전송
    - [x] 7.6.5 사용자가 `/status` 입력 시뮬레이션
    - [x] 7.6.6 검증: 봇 명령 우선 처리
    - [x] 7.6.7 사용자가 `ddd` 입력 시뮬레이션
    - [x] 7.6.8 검증: 백틱 명령 정상 처리
    - [x] 7.6.9 검증: 4단계 입력 처리 로직 정상 작동
  - [x] 7.7 응답 시간 로깅 및 성능 분석 추가
  - [x] 7.8 타임아웃 및 오류 조건 테스트 시나리오 추가
