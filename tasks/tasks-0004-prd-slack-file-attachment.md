...
- [x] 4.0 파일 첨부 핸들러 구현 (FR-1, FR-4, FR-5, FR-8, FR-9, FR-11)
  - [x] 4.1 `src/handlers/file-attachment.ts` 파일 생성 및 기본 인터페이스 정의
  - [x] 4.2 파일 첨부 감지 테스트 작성 (files 배열 존재 시 감지, files 없을 때 무시)
  - [x] 4.3 단일 파일 처리 테스트 작성 (1개 파일 처리, 여러 파일 중 첫 번째만 처리)
  - [x] 4.4 프롬프트 필수 검증 테스트 작성 (파일+프롬프트 정상, 파일만 에러)
  - [x] 4.5 프롬프트 생성 테스트 작성 (이미지: `@/path\n\nprompt`, 텍스트: 동일 형식)
  - [x] 4.6 handleFileAttachment() 함수 구현 (파일 검증 → 다운로드 → 프롬프트 생성 통합)
  - [x] 4.7 에러 메시지 테스트 작성 (타입 불일치, 크기 초과, 다운로드 실패, 프롬프트 없음)
  - [x] 4.8 에러 핸들링 구현 (FR-11의 모든 에러 케이스 처리)
